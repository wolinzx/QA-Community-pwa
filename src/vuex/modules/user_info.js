...
const mutations = {
    [types.SET_USERINFO] (state,{logined,data}){
      try{
        if(logined){
    ...
    [types.GET_USERINFO] (context){
      if(localStorage.user_info.get()){
        axios.get('/api/login/getAccount')
          .then((response)=>{
            if(response.data){
              context.commit('SET_USERINFO',{logined:true,data:localStorage.user_info.get()});
            }else{
              context.commit('SET_USERINFO',{logined:false});
            }
          })
          .catch((err)=>{
              throw err;
